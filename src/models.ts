...
export type Sprites =
  | (Record<
      | `${SpriteSide}_female`
      | `${SpriteSide}_shiny`
      | `${SpriteSide}_shiny_female`,
      string | null
    > &
      Record<`${SpriteSide}_default`, string>)
...
export interface Pokemon {
  ...
  held_items: {
    item: NamedApiRef
    version_details: { version: NamedApiRef; rarity: number }[]
  }[]
  id: number
  ...
  moves: {
    ...
    version_group_details: {
      ...
      move_learn_method: NamedApiRef
      version_group: NamedApiRef
    }[]
  }[]
  order: number
  species: NamedApiRef
